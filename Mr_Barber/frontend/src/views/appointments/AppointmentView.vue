...
<template>
    <h2 class="text-4xl text-white">Detalles Cita</h2>
    <p class="text-white text-lg">Verifica la informacion</p>

    <h3 class="text-3xl text-white"> Servicios </h3>
    ...
    <div class="space-y-8" v-if="!appointments.noServicesSelected">
        <h3 class="text-3xl text-white"> Fecha y Hora</h3>
        ...
        <div v-if="appointments.isValidReservation" class="flex justify-end">
            <button
                class="w-full md:w-auto bg-blue-500 p-3 uppercase text-white"
                @click="appointments.createAppointment"
            >
                Confirmar Resevacion
            </button>
        </div>
    </div>
</template>